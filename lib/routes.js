...
async function routes(fastify) {
	...
	fastify.post('/media', {beforeHandler: authRequired}, async request => {
		...
		await streamToPromise(request.multipart((field, file, filename, encoding, mimetype) => {
			...
			promises.push(api.Media.upload(name, file, mimetype).then(() => `https://api.catapult.inetwork.com/v1/users/${user.userId || process.env.BANDWIDTH_USER_ID}/media/${name}`));
		}, () => {}));
	...
	fastify.get('/messages/events', {beforeHandler: authRequired}, async (request, reply) => {
		...
		const {user} = request;
		await redis.subscribe(`message:${user.userId}:${user.applicationId}:${user.phoneNumber}:${user.servicePhoneNumber}`, `message:${user.userId || process.env.BANDWIDTH_USER_ID}:${user.applicationId}:${user.servicePhoneNumber}:${user.phoneNumber}`);
		reply.sse({});
